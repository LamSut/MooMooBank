...
public class TransferExternalPIN extends JFrame {
	 ...
	 */
	public static void main(String usrsend, String usrget,String bankget, String money) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					TransferExternalPIN frame = new TransferExternalPIN();
					...
					JLabel Name = new JLabel(mysql.exName(usrget,bankget));
					...
					OK.addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent e) {
							...
							if(getPIN.compareTo(correct)==0) {
								...
								mysql.ExTransfer(usrsend, usrget,bankget, amount);

								frame.dispose();
								frame.callPINyes(usrsend,usrget,bankget, mysql.getBalance(usrsend));
							}
							else {
					...
					Cancel.addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent e) {
							frame.dispose();
							frame.callExTransferNext(usrsend, mysql.getBalance(usrsend),bankget);
						}
					});
	...
	public void callExTransferNext(String usr, long bal, String bankget) {
		TransferExternalNext.main(usr, bal, bankget);
	}
